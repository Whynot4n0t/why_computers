...
public class glass_fiber_cable extends Block {
    ...
    private static final VoxelShape DOT_SHAPE = Block.createCuboidShape(7.25, 0, 7.25, 8.75, 1.5, 8.75);
    private static final Map<Direction, VoxelShape> field_24414 = Maps.newEnumMap(ImmutableMap.of(Direction.NORTH, Block.createCuboidShape(7.25, 0, 0, 8.75, 1.5, 16),
            Direction.SOUTH, Block.createCuboidShape(7.25, 0, 0, 8.75, 1.5, 16),
            Direction.EAST, Block.createCuboidShape(0, 0, 7.25, 16, 1.5, 8.75),
            Direction.WEST, Block.createCuboidShape(0, 0, 7.25, 16, 1.5, 8.75)));
    private static final Map<Direction, VoxelShape> field_24415 = Maps.newEnumMap(ImmutableMap.of(Direction.NORTH, VoxelShapes.union(field_24414.get(Direction.NORTH), Block.createCuboidShape(3.0, 0.0, 0.0, 13.0, 16.0, 1.0)),
    ...
    public glass_fiber_cable(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public void prepare(BlockState state, WorldAccess world, BlockPos pos, int flags, int maxUpdateDepth) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            if (!blockState.isOf(Blocks.OBSERVER)) {
                ...
                net.minecraft.block.RedstoneWireBlock.replace(blockState, blockState2, world, mutable, flags, maxUpdateDepth);
            }
            ...
            net.minecraft.block.RedstoneWireBlock.replace(blockState3, blockState4, world, mutable, flags, maxUpdateDepth);
        }
    }
    ...
    private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction, boolean bl) {
        ...
        if (bl && (bl2 = this.canRunOnTop(world, blockPos, blockState)) && connectsTo(world.getBlockState(blockPos.up()))) {
            if (blockState.isSideSolidFullSquare(world, blockPos, direction.getOpposite())) {
                return WireConnection.UP;
            }
            return WireConnection.SIDE;
        }
        if (connectsTo(blockState, direction) || !blockState.isSolidBlock(world, blockPos) && connectsTo(world.getBlockState(blockPos.down()))) {
            return WireConnection.SIDE;
        }
    ...
    private int increasePower(BlockState state) {
        return state.isOf(this) ? state.get(POWER) : 1;
    }
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block block, BlockPos fromPos, boolean notify) {
        ...
        if (state.canPlaceAt(world, pos)) {
            ...
            net.minecraft.block.RedstoneWireBlock.dropStacks(state, world, pos);
    ...
    private void addPoweredParticles(World world, Random random, BlockPos pos, Vec3d color, Direction direction, Direction direction2, float f, float g) {
        ...
        world.addParticle(new DustParticleEffect(new Vec3f(color), 2.0f), (double)pos.getX() + d, (double)pos.getY() + e, (double)pos.getZ() + k, 0.0, 0.0, 0.0);
    }
